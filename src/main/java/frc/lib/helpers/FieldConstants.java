...
// Lombok removed for compatibility; replaced with explicit constructor/getter below
    ...
    public AprilTagFieldLayout getLayout() {
      if (layout == null) {
        synchronized (this) {
          if (layout == null) {
            try {
              // Constants.disableHAL is not defined in this project; assume normal deploy
              Path p =
                  Path.of(
